...
The local PC can't train this. This guide gets the whole pipeline — **Train 2.5** (on-court
candidate masking + game-state features + capacity bump) plus the **full holdout prediction
run** — onto a rented GPU cheaply.

> **Train 2.5 in one line:** the model now (a) masks the actor + substitution heads to the
> on-court legal set in-graph (train/inference parity), and (b) sees the running game state —
> score, period, clock-in-period, team fouls. Both are derived from columns **already in the
> cleaned CSVs** at preprocess time, so there is **no clean-schema change** — a fresh re-clean is
> for hygiene only (§0), and `full_train.py train` re-preprocesses with the new features
> automatically. See `docs/next_train_observations.md` entries 01–04 for the reasoning.
...
## 0. Fresh re-clean — on your PC, before renting anything

The clean runs on your machine (it's data processing, not GPU training — `RawData/MasterFiles`
stays home). One command wipes stale cleaned data / vocabs / norm-stats / curriculum state,
re-cleans `RawData/MasterFiles -> data/season*.csv`, enriches with season context, and fits the
vocab + normalization stats over the full corpus:

```bash
# on your PC, from the repo root
python train_full.py init --fresh
```

This **does not train** (it's a wipe + clean + warmup-stats fit). It regenerates
`encoder/vocabs/*.json` — the pod clones those from git, so commit + push them so the pod's vocab
matches the freshly cleaned data:

```bash
git add encoder/vocabs data/processed/.gitkeep 2>/dev/null; git add -A encoder/vocabs
git commit -m "Re-clean: refresh vocabs for the Train 2.5 run"   # only if vocabs changed
git push origin main
```

> Train 2.5's weights land in `./artifacts_full2` (untouched by `init --fresh`, which only wipes
> the curriculum `./artifacts` + `reports/`). Archive `reports/` first if you want Train-1's eval
> baselines — `reports_train1_backup.zip` was already saved this session.

Then re-tar the cleaned CSVs for the pod (see §3) and continue.
...
Make sure `main` is pushed first (it carries Train 2.5 — the masking + game-state features — and
the freshly re-cleaned vocabs from §0):
```bash
# on your PC
git push origin main
```
...
> **"Can't I just ship the preprocessed tensors instead?"** Not cleanly. (1) Any local
> `data/processed/*.npz` predate Train 2.5 — they lack the game-state columns (and the on-court
> masking is in-graph, so old tensors would silently train the wrong model); `full_train.py train`
> regenerates them from `data/` anyway. (2) The prediction/eval
...
```bash
python full_train.py setup        # compute the mid-2023 cut + 100-game holdout (reuses §0's clean)
python full_train.py train        # Train 2.5: every head, fresh, recency-weighted -> ./artifacts_full2
python full_train.py eval-all     # predict ALL 100 holdout games straight through (no stopping)
```

- **`train`** re-preprocesses every head from `data/` (picking up the on-court masks + game-state
  features automatically), writes weights to `./artifacts_full2` and a per-head report under
  `reports/` tagged `full_train_2`. If it dies mid-run, just re-run `train` — it resumes at the
  next unfinished head.
...
| `STAGE_SIMS` | `config.py` | 21 | 25–31 | More sims per game → tighter averaged box scores + finer win-prob resolution (bumped from 11 for Train 2.5). |
...
## Notes
- Claude never launches training/rollout/cleaning — these commands are for you to run (the clean
  on your PC, the train/eval on the pod).
- Train 2.5 lives under `./artifacts_full2` / report tag `full_train_2`. (Train 1's
  `./artifacts_full` was deleted this session — architecture-incompatible with the new heads; the
  baselines it scored are preserved in `docs/next_train_observations.md`.)
- After the weights come back, the **inference dials** are the next lever (no retrain): re-fit
  `DELTA_TIME_SCALE` from `python -m simulation.diagnostics`, retune `PLAYER_TEMPERATURE` **down**
  from 2.0 (the on-court masking makes the head well-calibrated — expect ~1.0–1.3), and set
  `EVENT_BIAS` / `TYPE_BIAS` from the eval team-bias table if any residual remains.
